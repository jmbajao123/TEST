...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($req_id_post && $payment_status) {
        ...
        if (!in_array($payment_status, ['Unpaid', 'Paid'])) {
            ...
            // Update payment_status, reset c_is_read, insert OR number
            $stmt = $conn->prepare("UPDATE request SET payment_status = ?, c_is_read = 0, or_number = ? WHERE req_id = ?");
            $stmt->bind_param("sii", $payment_status, $or_number, $req_id_post);

            if ($stmt->execute()) {
                // Update $request_row to display updated OR number and status
                $request_row['payment_status'] = $payment_status;
                $request_row['or_number'] = $or_number;
                ...
                echo "<script>
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
                        text: 'Payment status updated and OR number generated!',
                        timer: 2000,
    ...
    <div class="col-lg-12">
        <div class="row justify-content-center">
            ...
            <div class="col-lg-4 mb-2">
                ...
                <h5><?php echo !empty($request_row['payment_status']) ? htmlspecialchars($request_row['payment_status']) : 'N/A'; ?></h5>
            </div>

            <!-- OR Number -->
            <div class="col-lg-4 mb-2">
                <label class="form-label">OR Number</label>
                <h5><?php echo !empty($request_row['or_number']) ? htmlspecialchars($request_row['or_number']) : 'N/A'; ?></h5>
            </div>
            <!-- OR Date -->
            <div class="col-lg-4 mb-2">
                ...
                <h5>
                    <?php 
                    if (!empty($request_row['or_date'])) {
                        echo htmlspecialchars(date('F j, Y', strtotime($request_row['or_date'])));
                    } else {
                        echo 'N/A';
                    }
                    ?>
            ...
            <div class="col-lg-12 mt-3">
                <a href="lsct.php" class="btn btn-outline-secondary mt-3">Back</a>
            </div>
        </div>
    </div>
</form>
