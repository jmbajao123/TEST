...
// --------------------
// 1. Student Notifications (Credential Code)
// --------------------
$query_students = "
    SELECT s_id, first_name, middle_name, last_name, suffix_name 
    FROM students 
    WHERE a_is_read = 0
    ORDER BY s_id DESC
";
...
while ($row = $result_students->fetch_assoc()) {
    $full_name = $row['first_name'] . ' ' .
                 ...
                 $row['last_name'] .
                 ((!empty($row['suffix_name']) && strtolower($row['suffix_name']) !== 'none') 
                    ? ' ' . $row['suffix_name'] 
                    : '');
    $notifications[] = [
        'type' => 'student',
        ...
        'message' => "Student Credential Code of <b>" . htmlspecialchars($full_name) . "</b> has been used."
    ];
}
...
// --------------------
$query_request = "
    SELECT r.req_id, r.status, s.first_name, s.middle_name, s.last_name, s.suffix_name
    FROM request r
...
while ($row = $result_request->fetch_assoc()) {
    ...
    $notifications[] = [
        ...
        'id' => $row['req_id'],
        'full_name' => trim($full_name),
        'message' => "Request from <b>" . htmlspecialchars($full_name) . "</b> is now <b>" . htmlspecialchars($row['status']) . "</b>."
    ];
}
?>
    ...
    <div class="dropdown-list-content dropdown-list-icons">
      <?php if (!empty($notifications)): ?>
        <?php foreach ($notifications as $notif): ?>
          <a href="<?= $notif['type'] === 'student' ? 'read_notif.php?s_id=' . $notif['id'] : 'read_request.php?req_id=' . $notif['id'] ?>" 
             class="dropdown-item dropdown-item-unread">
            <div class="dropdown-item-icon bg-info text-white">
              <i class="fas <?= $notif['type'] === 'student' ? 'fa-id-card' : 'fa-file-alt' ?>"></i>
            </div>
            <div class="dropdown-item-desc">
              <?= $notif['message'] ?>
            </div>
